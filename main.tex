...
\subsection{Extracting metadata from HTML and PDF targetting Wikidata}
The pyCEURmake ~\cite{pyCEURmake} tool was created to parse the main HTML index as well as the individual proceedings HTML pages vastly improving on the simple scraper created by Nielsen ~\cite{Nielsen2021} earlier.
The results have been cached in JSON and SQLite storage for quick access. Location entries have been consolidated using the ~\cite{geograpy3} library. Author, Editor and Paper details have been aligned with the DBLP SPARQL endpoint.
...
ceur-spt ~\cite{ceur-spt} is the prototype for showing the benefits of semantification by improved linking and navigation capababilities compared to the current static CEUR-WS HTML page.
...
\end{tabularx}
\end{table}
Figure \ref{fig:swat4hcls_2024_coauthor_graph} shows the user friendly display
of the coauthorship as graph visualization.
\begin{figure}[t]
    \centering
...
The move of the CEUR-WS infrastructure from RWTH Aachen to Gesellschaft for Informatik in 2026 is an opportunity to make further steps in this direction. The CEUR-WS semantify project ~\cite{CEUR-WSSemantify2026} is the open source effort pursuing this goal. 
