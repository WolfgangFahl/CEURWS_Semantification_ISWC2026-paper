...
Cermine ~\cite{Tkaczyk_2015} and GROBID \cite{Lopez_2009} have been run on the papers to provide additional metadata references.
...
semantification by improved linking and navigation capabilities compared
...
references) is available through the same interface that replicates the
CEUR-WS index (Figure~\ref{fig:ceurspt_index}). Per-volume pages expose
the consolidated metadata of each proceedings entry, as illustrated for
Vol-3262 in Figures~\ref{fig:ceurspt_vol3262_sample}
and~\ref{fig:ceurspt_vol3262_detail}.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{images/meeting_ceurspt_index_screenshot.png}
    \caption{ceur-spt index page at
    \url{https://ceurspt.wikidata.dbis.rwth-aachen.de/index.html} showing
    the re-synchronized CEUR-WS proceedings listing with a complete
    Cermine run.}
    \label{fig:ceurspt_index}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{images/meeting_ceurspt_vol3262_sample.png}
    \caption{CEUR-WS Volume 3262 Wikidata Workshop 2022 example with rich detail links }
    \label{fig:ceurspt_vol3262_sample}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{images/meeting_ceurspt_vol3262_detail.png}
    \caption{Detail excerpt of the Vol-3262 ceur-spt page: structured
...
aligning the digital, social and physical aspects, given that
...
example that makes the downstream indexing observable.
...
SWAT4HCLS~2024 (\wdq{Q124669329}{Q124669329}). All 12 panels survived the
...
CEUR-WS \textit{proceedings} items (\wdq{Q1143604}{Q1143604}). The long-tail
...
volume, e.g.
\url{http://ceurspt.wikidata.dbis.rwth-aachen.de/Vol-3264.jsonld?include_errors=false}.
...
The move of the CEUR-WS infrastructure from RWTH Aachen to Gesellschaft für Informatik in 2026 is an opportunity to make further steps in this direction. The CEUR-WS semantify project ~\cite{CEUR-WSSemantify2026} is the open source effort pursuing this goal. 
