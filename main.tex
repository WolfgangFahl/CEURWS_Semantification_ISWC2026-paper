...
\toprule
\# & Panel/Query & style & What the query contains \\
\midrule
1  & \texttt{data}                      && Core metadata: label, date, location, organisers, main subject \\
2  & \texttt{people}                    && Organisers, speakers, editors and participants \\
3  & \texttt{co-authors}                &graph& Co-author graph of people involved  \\
4  & \texttt{sponsors}                  && Sponsors of the event \\
5  & \texttt{timeline}                  &timeline& Timeline of related events \\
6  & \texttt{topic-scores}              && Topic scores derived from works at the event \\
7  & \texttt{uses}                      && Software, datasets and methods used \\
8  & \texttt{recent-publications}       && Recent publications by organisers, speakers, participants \\
9  & \texttt{proceedings}               && Works published in the proceedings related to the event \\
10 & \texttt{presentations}             && Talks, posters and presentations given at the event \\
11 & \texttt{related-events-timelocation} && Related events based on time and location \\
12 & \texttt{related-events-people}     && Related events based on people overlap \\
\bottomrule
\end{tabularx}
\end{table}
Figure \ref{fig:swat4hcls_2024_coauthor_graph} shows the user friendly display
of the coauthor ship as graph visualization.
\begin{figure}[t]
    \centering
